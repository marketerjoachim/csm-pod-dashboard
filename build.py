...
def compute_daily_churn(workspaces):
    """Compute yesterday's churn data grouped by pod for the Iron Herd Leaderboard."""
...
<div id="dashboard" class="locked max-w-[1500px] mx-auto px-4 sm:px-6 py-8">
  ...
  <div class="flex border-b border-dark-800 mb-6 fade-in">
    <button class="top-tab active" data-toptab="daily" onclick="switchTopTab('daily')">Iron Herd Leaderboard</button>
  ...
  <div id="tabDaily" class="tab-panel active fade-in">
    <div class="mb-6">
      <div class="flex items-center gap-4 mb-1">
        <h1 class="text-3xl sm:text-4xl font-800 text-white">Iron Herd Leaderboard</h1>
    ...
    <p class="text-dark-400 text-xs mt-5 text-center"><span class="text-dark-500 font-600">Customer Score</span> (estimated LTV potential): <span class="text-amber-400 font-600">&#9733; Gold</span> = highest &middot; <span class="text-dark-300 font-600">&#9733; Silver</span> = moderate &middot; <span class="text-dark-500 font-600">&#9733; Bronze</span> = low &middot; <span class="text-dark-600">n/a</span> = unrated</p>
  </div>
  ...
  <div class="flex flex-wrap gap-2 mb-6 fade-in" style="animation-delay:0.05s" id="podTabs">
    ...
    <button onclick="selectPod('Marcus+Martin')" class="pod-tab px-4 py-2 rounded-lg text-sm font-600 bg-dark-800/30 text-dark-500 border border-dark-800/30" data-pod="Marcus+Martin">Angus <span class="text-dark-600 text-xs">Perf &amp; Pro &middot; Marcus &amp; Martin</span> ({mm_cnt})</button>
    <button onclick="selectPod('Sebastian+Daniel')" class="pod-tab px-4 py-2 rounded-lg text-sm font-600 bg-dark-800/30 text-dark-500 border border-dark-800/30" data-pod="Sebastian+Daniel">Belgian Blue <span class="text-dark-600 text-xs">Pro &amp; Growth &middot; Sebastian &amp; Daniel</span> ({sd_cnt})</button>
    <button onclick="selectPod('Aimy+Espen')" class="pod-tab px-4 py-2 rounded-lg text-sm font-600 bg-dark-800/30 text-dark-500 border border-dark-800/30" data-pod="Aimy+Espen">Charolais <span class="text-dark-600 text-xs">Growth &middot; Aimy &amp; Espen</span> ({ae_cnt})</button>
    <button onclick="selectPod('Nicklas+Hamsa')" class="pod-tab px-4 py-2 rounded-lg text-sm font-600 bg-dark-800/30 text-dark-500 border border-dark-800/30" data-pod="Nicklas+Hamsa">Buffalo <span class="text-dark-600 text-xs">Growth &amp; Starter &middot; Nicklas &amp; Hamsa</span> ({nh_cnt})</button>
  </div>
    ...
    <div class="glass rounded-xl p-4">
      <div class="grid grid-cols-7 gap-2 items-center">
        <div class="flex items-center gap-2">
          ...
          <div>
            <p class="text-base font-700 text-white">${{POD_DISPLAY[pod]||pod}} <span class="text-dark-500 text-xs font-500 uppercase">${{POD_SEGMENT[pod]||''}}</span></p>
...
function copyDailyAsImage(){{
  const btn=document.getElementById('btnCopyImg');
  const origText=btn.innerHTML;
  btn.innerHTML='Rendering...';btn.disabled=true;
  // Build a self-contained snapshot div
  const wrap=document.createElement('div');
  wrap.style.cssText='position:fixed;left:-9999px;top:0;width:900px;padding:32px;background:#0f172a;color:#e2e8f0;font-family:Inter,system-ui,sans-serif;';
  // Title + date
  const hdr=document.createElement('div');
  hdr.style.marginBottom='16px';
  hdr.innerHTML='<div style="font-size:24px;font-weight:800;color:#fff;">Iron Herd Leaderboard</div><div style="font-size:13px;color:#64748b;margin-top:4px;">Feb 17, 2026</div>';
  wrap.appendChild(hdr);
  // Clone aggregate + cards
  const ag=document.getElementById('dailyAggregate').cloneNode(true);
  ag.style.marginBottom='12px';
  wrap.appendChild(ag);
  const cards=document.getElementById('dailyCards').cloneNode(true);
  wrap.appendChild(cards);
  document.body.appendChild(wrap);
  html2canvas(wrap,{{backgroundColor:'#0f172a',scale:2,useCORS:true}}).then(function(canvas){{
    canvas.toBlob(function(blob){{
      if(navigator.clipboard&&navigator.clipboard.write){{
        navigator.clipboard.write([new ClipboardItem({{'image/png':blob}})])
          .then(function(){{btn.innerHTML='&#10003; Copied!';setTimeout(function(){{btn.innerHTML=origText;btn.disabled=false;}},2000);}})
          .catch(function(){{fallbackDownload(canvas,btn,origText);}});
      }}else{{fallbackDownload(canvas,btn,origText);}}
    }},'image/png');
    document.body.removeChild(wrap);
  }}).catch(function(){{btn.innerHTML=origText;btn.disabled=false;document.body.removeChild(wrap);}});
}}
function fallbackDownload(canvas,btn,origText){{
  const a=document.createElement('a');a.href=canvas.toDataURL('image/png');a.download='iron-herd-leaderboard.png';a.click();
  btn.innerHTML='&#10003; Downloaded!';setTimeout(function(){{btn.innerHTML=origText;btn.disabled=false;}},2000);
}}
