...
function renderDailyCards(){{
  ...
  agEl.innerHTML=`
    <div class="grid grid-cols-7 gap-2 items-center">
      ...
      <div class="text-center"><p class="text-xl font-700 ${{agChurnColor}}">${{totChurnCnt>0?totChurnCnt:'0'}}</p><p class="text-dark-400 text-xs font-600">Churned</p>${{totChurnCnt>0?'<p class="text-red-400/70 text-xs">'+fmtChurnArr(totChurnArr)+' MRR</p>':''}}</div>
      <div class="text-center"><p class="text-xl font-700 ${{totRenCnt>0?'text-emerald-400':'text-dark-500'}}">${{totRenCnt>0?totRenCnt:'0'}}</p><p class="text-dark-400 text-xs font-600">Renewed</p>${{totRenCnt>0?'<p class="text-emerald-400/70 text-xs">'+fmtChurnArr(totRenMrr)+' MRR</p>':''}}</div>
  ...
  el.innerHTML=podOrder.map((pod,idx)=>{{
    ...
    <div class="glass rounded-xl p-4">
      <div class="grid grid-cols-7 gap-2 items-center">
        ...
        <div class="text-center"><p class="text-base font-700 ${{churnColor}}">${{hasCh?d.churnCnt:'0'}}</p><p class="text-dark-500 text-xs">Churned</p>${{hasCh?'<p class="text-red-400/70 text-xs">'+fmtChurnArr(d.churnArr)+' MRR</p>':''}}</div>
