...
<div id="dashboard" class="locked max-w-[1500px] mx-auto px-4 sm:px-6 py-8">
  ...
  <div class="flex flex-wrap gap-2 mb-6 fade-in" style="animation-delay:0.05s" id="podTabs">
    ...
    <button onclick="selectPod('Marcus+Martin')" class="pod-tab px-4 py-2 rounded-lg text-sm font-600 bg-dark-800/30 text-dark-500 border border-dark-800/30" data-pod="Marcus+Martin">Marcus + Martin ({mm_cnt})</button>
    <button onclick="selectPod('Sebastian+Daniel')" class="pod-tab px-4 py-2 rounded-lg text-sm font-600 bg-dark-800/30 text-dark-500 border border-dark-800/30" data-pod="Sebastian+Daniel">Sebastian + Daniel ({sd_cnt})</button>
    <button onclick="selectPod('Aimy+Espen')" class="pod-tab px-4 py-2 rounded-lg text-sm font-600 bg-dark-800/30 text-dark-500 border border-dark-800/30" data-pod="Aimy+Espen">Aimy + Espen ({ae_cnt})</button>
    <button onclick="selectPod('Nicklas+Hamsa')" class="pod-tab px-4 py-2 rounded-lg text-sm font-600 bg-dark-800/30 text-dark-500 border border-dark-800/30" data-pod="Nicklas+Hamsa">Nicklas + Hamsa ({nh_cnt})</button>
  </div>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="text-dark-400 text-xs uppercase tracking-wider border-b border-dark-800" id="headerRow">
            ...
            <th class="text-center px-2 py-3 font-600"><span class="tip">A<span class="tip-text">Open in Attio.</span></span></th>
          </tr>
        </thead>
...
function podLabel(p){{return p.split('+').map(x=>x[0]).join('');}}
...
function stripeLink(wsId){{if(!wsId)return'<span class="text-dark-700">&mdash;</span>';return`<a href="https://dashboard.stripe.com/subscriptions/${{wsId}}" target="_blank" rel="noopener" class="link-icon text-dark-400 hover:text-blue-400" title="Open in Stripe">\u21D7</a>`;}}
function attioLink(recId){{if(!recId)return'<span class="text-dark-700">&mdash;</span>';return`<a href="https://app.attio.com/metric/workspaces/record/${{recId}}/overview" target="_blank" rel="noopener" class="link-icon text-dark-400 hover:text-purple-400" title="Open in Attio">\u21D7</a>`;}}
...
function renderTable(){{
  ...
  tbody.innerHTML=filtered.map((w,i)=>`
    <tr class="workspace-row border-b border-dark-800/50 ${{tierCls(w.t)}}">
      ...
      <td class="px-3 py-2.5 font-600 text-white text-sm whitespace-nowrap">${{w.n}}</td>
