...
<div id="dashboard" class="locked max-w-[1500px] mx-auto px-4 sm:px-6 py-8">
  ...
  <div class="glass rounded-xl overflow-hidden fade-in" style="animation-delay:0.25s">
    ...
    <div class="overflow-x-auto scrollbar-thin" id="tableScroll">
      <table class="text-sm" id="mainTable" style="min-width:1400px">
        <thead>
...
function initColResize(){{
  ...
  requestAnimationFrame(function(){{
    // Snapshot natural widths, shrink # col by 25%
    const widths=[];
    ths.forEach(function(th,i){{ widths.push(th.offsetWidth); }});
    if(widths[0])widths[0]=Math.round(widths[0]*0.75);
    // Apply fixed layout with measured widths
    ...
    ths.forEach(function(th,i){{
      ...
      handle.addEventListener('mousedown',function(e){{
        ...
        const nTh=ths[i+1];
        const nW=nTh?nTh.offsetWidth:0;
        ...
        function onMove(e2){{
          ...
          if(nTh)nTh.style.width=Math.max(30,nW-dx)+'px';
        }}
        function onUp(){{
